...
class EmpleadoDTO:
    ...
    def buscarEmpleado(self, empleado):  # ✅ CORREGIDO: recibe objeto
        resultado = self.dao.findEmpleado(empleado.getRun())  # ✅ usa getter
        if resultado:
    ...
    def validarLogin(self, empleado):  # ✅ CORREGIDO: recibe objeto
        empleado_completo = self.dao.findEmpleado(empleado.getRun())  # ✅ usa getter
        
        if empleado_completo:
            if Encoder().decode(empleado.getPassword(), empleado_completo[5]):  # ✅ usa getter
    ...
    def agregarEmpleado(self, empleado):  # ✅ CORREGIDO: recibe objeto
        # Encriptar password antes de guardar
        empleado.setPassword(Encoder().encode(empleado.getPassword()))
        resultado = self.dao.addEmpleado(empleado)
        return resultado
    
    def actualizarEmpleado(self, empleado):  # ✅ CORREGIDO: recibe objeto
        # Encriptar password antes de actualizar
        empleado.setPassword(Encoder().encode(empleado.getPassword()))
        resultado = self.dao.updateEmpleado(empleado)
        return resultado
    
    def eliminarEmpleado(self, empleado):  # ✅ CORREGIDO: recibe objeto
        resultado = self.dao.deleteEmpleado(empleado.getRun())  # ✅ usa getter
        return resultado